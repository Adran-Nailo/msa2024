...
#wn.register_shape("textures/water.gif")
...
def new_player(file):
    f = open(file,"w")
    f.write("11*9*0*[0]*world_map*20*5*0*0*0*3*False*False*0*False*False*0*"+file)
    f.close()
...
def load_save_data(file):
    ...
    player = characters.Characters(int(split_data[0]),int(split_data[1]),score,split_data[3],map_class,int(split_data[5]),int(split_data[6]),int(split_data[7]),int(split_data[8]),int(split_data[9]),int(split_data[10]),split_data[11],split_data[12],int(split_data[13]),split_data[14],split_data[15],int(split_data[16]),split_data[17])
    return player

def save_file_contents():
    f = open(player.save_file, "w")
    map_class = map_saver()
    #print(f"{player.coordinates_x}, {player.coordinates_y}")
    f.write(f"{player.coordinates_x}*{player.coordinates_y}*{player.score}*{player.icon}*{map_class}*{player.health}*{player.damage}*{player.sword_count}*{player.potion_count}*{player.wall_count}*{player.render_distance}*{player.basic_attack_unlocked}*{player.advanced_attack_unlocked}*{player.armor}*{player.basic_shield}*{player.advanced_shield}*{player.block}*{player.save_file}")
    f.close
...
def main():
    ...
    while end_game != True:
        ...
        y_offset = 0
        if left_bound < 0:
            x_offset += 25*left_bound
            left_bound = 0
            right_bound += 1
        if right_bound >= width:
            x_offset += 25*(right_bound-width)
            right_bound = len(player.current_map)-1
            left_bound -= 1
        if upper_bound < 0:
            y_offset -= 50*upper_bound
            upper_bound = 0
            lower_bound += 1
        if lower_bound >= len(player.current_map):
            y_offset -= 50*(lower_bound-len(player.current_map))
        ...
        if skip_map == False:
            ...
            for row in range(upper_bound,lower_bound+1):
                ...
                start_y = (lower_bound - upper_bound)*25 - current_tile_y_pos*50+25
                current_tile_x_pos = 0
                for length in range(left_bound, right_bound):
                    start_x = (right_bound - left_bound)*-25 + current_tile_x_pos*50
                    ...
                    if player.current_map[row][length] in tiles:
                        ...
                        current_tile_color = "deep pink"
                    elif player.current_map[row][length] in containers:
                        current_tile_color = "blue violet"
                    ...
                    square()
                    t.end_fill()
                    if player.current_map[row][length] == "H20":
                        t.shape("textures/water.gif")
                        t.seth(0)
                        t.fd(25)
                        t.rt(90)
                        t.fd(25)
            ...
            end_game = wn.onkeypress(save_file_contents,"`")
