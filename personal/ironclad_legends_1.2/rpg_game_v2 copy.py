...
#define map turtle and screen
t = turtle.Turtle()
t.hideturtle()
wn = turtle.Screen()
player_icon = turtle.Turtle()
player_icon.shapesize(2)
t.speed(0)
player_icon.goto(0,0)
player_icon.penup()
player_icon.shape("circle")
wn.tracer(0)


image_files = os.listdir("textures/minecraft_source")
for file in image_files:
    without_extn = os.path.splitext(file)[0]
    img = Image.open("textures/minecraft_source/"+file)  # or .jpg
    img = img.resize((50, 50), Image.NEAREST)
    img.save("textures/minecraft/"+without_extn+".gif")  # Turtle only accepts .gif

image_gifs = os.listdir("textures/minecraft")
for file in image_gifs:
    wn.register_shape("textures/minecraft/"+file)
filepath = "textures/minecraft/"
...
def new_player(file):
    f = open(file,"w")
    f.write("9*11*0*[0]*world_map*20*5*0*0*0*3*False*False*0*False*False*0")
    f.close()
...
def map_loader(map_string):
    possible_map_strings = ["world_map","h01","dungeon1","water_map"]
    map_codes = [maps.world_map,maps.h01,maps.dungeon1,maps.water_map]
...
def load_save_data(file):
    ...
    player = characters.Characters(int(split_data[0]),int(split_data[1]),score,split_data[3],map_class,int(split_data[5]),int(split_data[6]),int(split_data[7]),int(split_data[8]),int(split_data[9]),int(split_data[10]),split_data[11],split_data[12],int(split_data[13]),split_data[14],split_data[15],int(split_data[16]),"test.txt")
    return player
...
def interact():
    ...
    for row in range(upper_bound,lower_bound + 1):
            for length in range(left_bound, right_bound + 1): 
                if player.current_map[row][length] in interactable_objects:
                    current_interactable_object = player.current_map[row][length]
...
def move_up():
    ...
    if player.current_map[player.coordinates_y][player.coordinates_x] in hazards:
        player.score -= 1
    if player.current_map[player.coordinates_y][player.coordinates_x] in solids:
        player.coordinates_y += 1
    if (player.current_map[player.coordinates_y][player.coordinates_x] == "[c]"):
        player.score += 1
        print("You fould 1 gold!")
        player.current_map[player.coordinates_y][player.coordinates_x] = "[ ]"
    if player.coordinates_y < 0:
        player.coordinates_y = 0
    player.updaterender = True

def move_right():
    player.coordinates_x += 1
    if player.current_map[player.coordinates_y][player.coordinates_x] in hazards:
        player.score -= 1
    if player.current_map[player.coordinates_y][player.coordinates_x] in solids:
        player.coordinates_x -= 1
    if (player.current_map[player.coordinates_y][player.coordinates_x] == "[c]"):
        ...
        print("You fould 1 gold!")
        player.current_map[player.coordinates_y][player.coordinates_x] = "[ ]"
    if player.coordinates_x >= len(player.current_map[0]):
        player.coordinates_x = len(player.current_map[0])-1
    player.updaterender = True
...
def move_down():
    ...
    if (player.current_map[player.coordinates_y][player.coordinates_x] == "[c]"):
        ...
        print("You fould 1 gold!")
        player.current_map[player.coordinates_y][player.coordinates_x] = "[ ]"
    if player.coordinates_y >= len(player.current_map):
        player.coordinates_y = len(player.current_map)-1
    player.updaterender = True
...
def move_left():
    ...
    if (player.current_map[player.coordinates_y][player.coordinates_x] == "[c]"):
        ...
        print("You fould 1 gold!")
        player.current_map[player.coordinates_y][player.coordinates_x] = "[ ]"
    if player.coordinates_x < 0:
        player.coordinates_x = 0
    player.updaterender = True
    
def place_wall():
    place_item(player.current_map,"[w]")

def place_hole():
    place_item(player.current_map,"[ ]")
...
def colored_square(current_color):
...
def main():
    ...
    while end_game != True:
        ...
        tiles = ["[t]","[c]","[C]","[f]","[w]","H2O","[b]","gcw","[ ]","[W]","   "]
        colors = ["dark green","yellow","cyan","red","red","blue","silver","dim gray","green","light sky blue","black"]
        tile_textures = [filepath+"trees.gif",filepath+"coins.gif",filepath+"misingno.gif",filepath+"misingno.gif",filepath+"misingno.gif",filepath+"misingno.gif",filepath+"misingno.gif",filepath+"misingno.gif",filepath+"grass.gif",filepath+"misingno.gif",filepath+"misingno.gif"]
        x_offset = 0
        y_offset = 0
        if left_bound < 0:
            x_offset += 50*left_bound
            left_bound = 1
        if right_bound >= width:
            x_offset += 50*(right_bound-width)
            right_bound = len(player.current_map)-1
        if upper_bound < 0:
            y_offset -= upper_bound*25
            upper_bound = 0
        if lower_bound >= len(player.current_map):
            y_offset -= 25*(lower_bound-len(player.current_map))
        ...
        if skip_map == False:
            tile_amount_count = player.render_distance*2+1
            current_tile_y_pos = 0
            for row in range(upper_bound,lower_bound+1):
                
                start_y = (lower_bound - upper_bound)*25 - current_tile_y_pos*50+25-25
                current_tile_x_pos = 0
                for length in range(left_bound, right_bound):
                    start_x = (right_bound - left_bound)*-25 + current_tile_x_pos*50+25

                    #Render and color partial tiles
                    t.hideturtle()
                    t.up()
                    t.goto(start_x,start_y)
                    current_tile_color = filepath+"misingno.gif"
                    if player.current_map[row][length] in tiles:
                        current_tile_color = tile_textures[tiles.index(player.current_map[row][length])]
                    ...
                    t.stamp()
                    current_tile_x_pos += 1

                    
                current_tile_y_pos += 1
            player_icon.goto(x_offset,y_offset)
            wn.onkeypress(move_up,"w")       
            wn.onkeypress(move_down,"s")   
            wn.onkeypress(move_left,"a")   
            wn.onkeypress(move_right,"d")
            wn.onkeypress(place_wall,"p")
            wn.onkeypress(place_hole,"h")
...
while True:
        ...
        try:
            ...
            continue
        while True:
            new_file = input("Is this a new file? y/n\n")
            if new_file == "y":
